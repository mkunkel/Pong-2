...
exports.findPlayerById = function(playerId, fn){
...
exports.savePhoneSocket = function(player, phoneId, fn){
  console.log(player);
  player.phoneId = phoneId;
  player.save(function(err, player){
    fn(err, player);
  });
};

exports.updatePaddles = function(sockets, game, paddles, locations, fn) {
  console.log(paddles);

  for(var i = 0; i < game.players.length; i++){
    if(sockets[game.players[i].socketId]){
      // update paddle locations based on player latency
      // for(var n = 0; n < game.players[i].latency; n += 2){
      //   if (paddles[0] !== null) {locations[0] += paddles[0];}
      //   if (paddles[1] !== null) {locations[1] += paddles[1];}
      // }

      sockets[game.players[i].socketId].emit('updatepaddles', {paddles: paddles, locations: locations});
    }
  }
  fn();
};
...
exports.saveLatency = function(player, time, fn) {
  player.latency = Math.floor((Date.now() - time) / 25);
  player.save(function(err, player) {
    fn();
  });
};
...
exports.emitPlayers = function(sockets, game, fn){
  ...
  for(var i = 0; i < game.players.length; i++){
    ...
    // also send info to phones
    console.log(game.players[i].phoneId);
    if(sockets[game.players[i].phoneId]){
      sockets[game.players[i].phoneId].emit('playerjoined', {game:game, players:game.players});
    }
  }
  fn();
};
